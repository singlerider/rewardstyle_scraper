...
def rstyle_search(advertiser): # do a search for a given advertiser
    url = "https://api.rewardstyle.com/v1/search?oauth_token={}&advertisers[]={}".format(
        access_token, advertiser)
    resp = requests.get(url)
    data = json.loads(resp.content) # if there is a result, grab the json
    product_id = data["products"][0]["product_id"] # snag the product id of the top result
    return product_id
...
def rstyle_link(advertiser): # gets a shortlink
    try:
        ...
        return error
    url = "https://api.rewardstyle.com/v1/get_link?oauth_token={}&product={}".format(
        access_token, rstyle_search(advertiser))
    resp = requests.get(url)
    data = json.loads(resp.content) # this is the json for the shortlink request
...
def text_cleaner(stripped_advertiser, website): # clean up html and read words
    ...
    try:
        ...
        return # just in case there are some weird characters here, don't kill app
    text = re.sub("[^a-zA-Z.+3]"," ", text) # only regular words, no numbers
    text = text.lower().split() # lower case so the dict can be properly populated
    stop_words = set(stopwords.words("english"))
    text = [w for w in text if not w in stop_words] # gets rid of words like "and"
...
def scrape():
    ...
    for i in range(len(advertisers)): # per advertiser
        ...
        if "and" in advertiser:
            advertiser = advertiser.split("and")[0].strip(" ") # separate multiple
            # advertisers and search for only the first listed
        if ".com" in advertiser:
            advertiser = advertiser.replace(".com", "") # could prevent search result
        stripped_advertiser = advertiser.replace(" ", "_") # get rid of spaces for file name
        ...
        try:
            ...
            short_data = short_resp.content.encode("utf-8") # generate unicode data
        except Exception as error:
    ...
    final_frame = pd.DataFrame(overall_total_skills.items(), columns = ["Term",
        "NumPostings"])
    pd.set_option('display.height', 500000) # this is just an arbitrary maximum
